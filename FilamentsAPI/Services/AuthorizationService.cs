...
namespace FilamentsAPI.Services
{
    ...
    public class AuthorizationService(IConfiguration configuration, IOptions<AppSettings> appSettings) : IAuthorizationService
    {
        ...
        private void EnsureLoggedInUserIsAdminOrSelf(FilamentsAPIDbContext db, ClaimsPrincipal loggedInUser, int? id)
        {
        ...
        private bool IsTargetUSerSameAsLoggedInUser(FilamentsAPIDbContext db, ClaimsPrincipal loggedInUser, int? id)
        {
        ...
        private void EnsureLoggedInUserIsAdmin(FilamentsAPIDbContext db, ClaimsPrincipal loggedInUser)
        {
            if (!loggedInUser.Claims.Any(c => c.Type == TokenService.IsAdministratorClaim))
            {
                throw new UnauthorizedAccessException($"Not allowed");
