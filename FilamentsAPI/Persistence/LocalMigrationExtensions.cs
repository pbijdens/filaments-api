    ...
    public static class LocalMigrationExtensions
    {
        ...
        public static async Task MigrateDatabases(this WebApplication? app)
        {
            ...
            try
            {
                ...
                {
                    using FilamentsAPIDbContext context = new(configuration);
                    await context.Database.MigrateAsync();
                }
                scope.ServiceProvider.GetService<ILogger<FilamentsAPIDbContext>>()?.LogInformation("Database migrated succesfully.");
                ...
                scope.ServiceProvider.GetService<ILogger<FilamentsAPIDbContext>>()?.LogInformation("Database seeded succesfully.");
            }
            catch (Exception ex)
            {
                await using var scope = app.Services.CreateAsyncScope();
                scope.ServiceProvider.GetService<ILogger<FilamentsAPIDbContext>>()?.LogError(ex, "Failed to apply migrations.");
        ...
        public static async Task SeedDatabaseIfNeeded(IConfiguration configuration)
        {
            ...
            string username = configuration.GetSection("AppSettings").GetValue<string>("DefaultUser") ?? "admin";
